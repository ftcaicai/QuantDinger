...
- **Hyperliquid 符号搜索**：`/api/market/symbols/search?market=Crypto&exchange_id=hyperliquid` 走 CCXT fallback（Binance USDT 对），`_search_crypto_exchange` 用 `display_quote='USDC'` 把 symbol 改写为 `BASE/USDC`（market 标签仍是 `Crypto`）。HL 独占 token（HYPE、PURR）v1 搜不到——种子表 + Binance 都没有。修复要等 P2 拉 HL 自己的 `Info.meta()` / `spot_meta()` universe。
