...
- **Hyperliquid 符号搜索**：`/api/market/symbols/search?market=Hyperliquid` 走与 Crypto 同样的 CCXT fallback（Binance USDT 对），但 `_search_crypto_exchange` 接受 `display_market='Hyperliquid'` + `display_quote='USDC'`，把结果 symbol 改写为 `BASE/USDC` + market 标签为 `Hyperliquid`。HL 独占 token（HYPE、PURR）在 v1 搜索里**搜不到**——种子表 + Binance 都没有。修复要等 P2 拉 HL 自己的 `Info.meta()` / `spot_meta()` universe。
