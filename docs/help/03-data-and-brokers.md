...
- **HL 走完整 limit-first 流程**：`HyperliquidClient` 已实现 `place_limit_order` + `wait_for_fill` + `cancel_order`；`pending_order_worker` 在 5 个 dispatch 点（限价下单 / 等成交 / 撤单 / 市价 fallback / 市价等成交）都加了 HL 分支。`order_mode='maker'` / `'limit_first'` 时 HL 用 ALO TIF（HL 在会成交时拒单，等同于 post-only）。
- **HL Quick-Trade 全开**：`/balance`、`/position`、`/close-position`、`/place-order` 四个端点都支持（HL `marginSummary.accountValue` / `[{position: {...}}]` 在 `_parse_balance` / `_fetch_exchange_positions_raw` 里展平；`/place-order` 用 HL `get_ticker` 反算 USDT→qty，与 Binance/OKX 同一套通用流程）。
...
- **Hyperliquid 是 Crypto market 下的交易所**，不是独立 market type。`/types`、`_MARKET_ALIASES` 都不含 Hyperliquid。`maybe_transform_kline_symbol` 仅在 `exchange_id='hyperliquid'` 且 `market='Crypto'` 时触发——通过策略凭据的 `exchange_config.exchange_id` 自动带入。
- **Hyperliquid 符号搜索**：`/api/market/symbols/search?market=Crypto&exchange_id=hyperliquid` 走 CCXT fallback（Binance），默认 quote = `USDC`（HL 计价习惯）。**关键字里的显式 quote 优先生效**：`btc/usdc` → `BTC/USDC`，`btc/usdt` → `BTC/USDT`，裸 `btc` → 走 caller 的 default（HL 是 USDC，普通 Crypto 是 USDT）。market 标签始终是 `Crypto`。
- **`_search_crypto_exchange` 缓存按 quote 分桶**：`_crypto_markets_cache: Dict[quote, {data, ts}]`，每个 quote 4 小时 TTL。`_SEARCHABLE_QUOTES = ('USDT', 'USDC')`——搜其它 quote 会回退到 default 避免缓存爆炸。HL 独占 token（HYPE、PURR）v1 搜不到，要等 P2 拉 HL 自己的 `Info.meta()` / `spot_meta()` universe。
